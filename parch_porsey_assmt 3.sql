...
SELECT web_events.occurred_at AS time_of_event, web_events.channel, 
...
-- the order. Your final table should have 3 columns: region name, account 
-- name, and unit price. A few accounts have 0 for total, so I divided 
-- by (total + 0.01) to assure not dividing by zero.???

SELECT region.name, accounts.name, 
total_amt_usd / (total + 0.01) AS unit_price
FROM accounts
OUTER JOIN orders ON accounts.id = orders.account_id
JOIN region ON region.id = accounts.id;

-- 4. Provide a table that provides the region for each sales_rep 
-- along with their associated accounts. This time only for the Midwest 
...
--5. Provide a table that provides the region for each sales_rep along 
...
-- 6. Provide a table that provides the region for each sales_rep 
...
-- 7. Provide the name for each region for every order, as well as the 
-- account name and the unit price they paid (total_amt_usd/total) for 
-- the order. However, you should only provide the results if the standard 
-- order quantity exceeds 100. Your final table should have 3 columns: 
-- region name, account name, and unit price. In order to avoid a division by
...
-- 8. Provide the name for each region for every order, as well as 
-- the account name and the unit price they paid (total_amt_usd/total) for the order. 
...
-- 9. What are the different channels used by account id 1001? Your final 
